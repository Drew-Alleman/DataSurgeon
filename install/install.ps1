...
if (-not(Test-Path -Path "C:/ds/" -PathType Leaf)) {
  Write-Host -Message "[*] Creating C:/ds/ to store the executable"
  mkdir C:/ds/ | Out-Null
}
Write-Host -Message "[*] Binding ds.exe to path (requires admin)"
