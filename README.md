...
# Speed Tests
When no specific query is provided, ds will search through all possible types of data, which is <b>SIGNIFICANTLY</b> slower than using individual queries. The files regex is the slowest query.'
...
Command         | Speed
----------------|----------------
`cat test.txt \| ds -t -6` | 00h:00m:20s
`cat test.txt \| ds -t -i -m` | 00h:00m:31s
`cat test.txt \| ds -t -F -6 -c` | 00h:00m:42s
